...
$t = time();
$t2 = $t - (60 * 60 * 24 * 30); //1 Monat zur�ck
...
if($query_topics->numRows() > 0) {
	...
	while($topic = $query_topics->fetchRow()) {
		...
		<tr class="row_<?php echo $trclass; ?>" valign="top">
		<td width="40" align="right"><?php

		echo '<a name="tpid'.$t_id.'" id="tpid'.$t_id.'">';
		...
		echo $modifycommentslink;
		echo $modifylink;
		$alt='ACTIVE_'.$active; echo '<img src="'.$picsurl.'active'.$active.'.gif" alt="'.$MOD_TOPICS[$alt].'" /></a>';
		?>
		</td>
		<td>
		<?php
			...
			echo '<strong>'.$modifylink.$title.'</a></strong>'; if ($topic['short_description'] !='') {echo '<div class="shortdesc">'.$topic['short_description'].'</div>';}
		?></td>
		<td class="topicprops">
		...
		if ($use_timebased_publishing > 1) {
			...
			echo '<td width="20">'.$modifylink.'<img src="'.$icon.'" border="0" alt="" /></a></td>';
		} ?>

		<?php
		if ($showsortarrows == true) {
			echo '<td width="20">'; if($counter > 1) { echo '<a href="'.WB_URL.'/modules/'.$mod_dir.'/move.php?'.$params.$paramdelimiter.'move=up" title="'.$TEXT['MOVE_UP'].'"><img src="'.THEME_URL.'/images/up_16.png" border="0" alt="^" /></a>';} echo '</td>';
			echo '<td width="20">'; if($counter < $num_topics) { echo '<a href="'.WB_URL.'/modules/'.$mod_dir.'/move.php?'.$params.$paramdelimiter.'move=down" title="'.$TEXT['MOVE_DOWN'].'"><img src="'.THEME_URL.'/images/down_16.png" border="0" alt="v" /></a>';} echo '</td>';
		...
		if ($topic['hascontent'] > 0 AND $active > 0) {
			$topic_link = WB_URL.$topics_directory.$topic['link'].PAGE_EXTENSION;
		 	echo '<a href="'.$topic_link.'" target="_blank" ><img src="'.THEME_URL.'/images/view_16.png" class="viewbutton" alt="View" /></a>';
		} ?>
		</td>
		</tr>
		<?php
