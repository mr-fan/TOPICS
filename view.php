...
if ($is_master == '') {
	...
	if (is_numeric(trim($is_master_Arr[0]))) {
		...
		if (!is_numeric(trim($is_master_Arr[0]))) {
			$theq = "SELECT section_id FROM ".TABLE_PREFIX."mod_topics_settings WHERE picture_dir = '".$settings_fetch['picture_dir']."'";
...
if(!isset($settings_fetch['picture_values'])){
	...
	$vv = explode(',','-2,-2,-2,-2,-2,-2,-2,-2,-2');
} else {
	$vv = explode(',',$settings_fetch['picture_values'].',-2,-2,-2,-2,-2,-2,-2,-2,-2');
} 
